...
public class CompressorInfoMenu extends PaginatedMenu {
    ...
    public void updateLore(ItemStack ore, String oreType, int oreIndex) {
        ...
        if (!lore.contains(prefix.setHexStringFormat("&aRequired To Upgrade: &6%d Pieces", cost))) {
            ...
            lore.set(1, " ");
            lore.add(prefix.setHexStringFormat("&aRequired To Upgrade: &6%d Pieces", cost));
            lore.add(" ");
            lore.add(prefix.setHexStringFormat("&cNeeds &6%d &cMore to Upgrade", totalAmount));
        }

...
public class OthersShopMenu  extends PaginatedMenu {

    Plugin plugin = HiveminezSeason2.getPlugin();
    Prefix prefix = HiveminezSeason2.getPrefix();

    public OthersShopMenu(PlayerMenuUtility playerMenuUtility) {
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        if (e.getCurrentItem().getType().equals(Material.BARRIER)) {
            HiveminezSeason2.getPlugin().getServer().getScheduler().runTask(HiveminezSeason2.getPlugin(), () -> new ShopMenu(HiveminezSeason2.getPlayerMenuUtility(p)).open());
        }
    ...
    private void updateLore(ItemStack tool, String toolType, List<ItemStack> ores, int toolIndex, List<ItemStack> tools) {
        ...
        try {
            if (!lore.contains(prefix.translateColorCodes("&7&nRequired Cost:"))) {
                ...
                lore.add(prefix.setHexStringFormat("&f- &a%s &fx1", Objects.requireNonNull(tools.get(index).getItemMeta()).getDisplayName()));
                lore.add(prefix.setHexStringFormat("&f- &6%s &fx2", Objects.requireNonNull(ores.get(index).getItemMeta()).getDisplayName(),
                        plugin.getConfig().getInt(String.format("shop.tools.%s.%d", toolType, toolIndex))));
                lore.add(" ");
                lore.add(prefix.translateColorCodes("&eReward:"));
                lore.add(prefix.setHexStringFormat("&f- &a%s &fx1", Objects.requireNonNull(tools.get(index + 1).getItemMeta()).getDisplayName()));
                lore.add(" ");
